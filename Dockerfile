...
RUN yum install -y epel-release centos-release-scl yum-utils && yum groupinstall -y "Development Tools" && yum install -y git wget curl libstdc++-devel make mesa-libGL-devel fontconfig devtoolset-7-gcc devtoolset-7-binutils devtoolset-7-gcc-c++ \
